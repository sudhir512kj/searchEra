...
if __name__ == "__main__":
    ...
    urls = soup_data.findAll(
        'a', attrs={'class': ' topic-grid-item js-topic-grid-item'})
    for data in urls:
        ...
        for result in table:
            ...
            d3 = result.findAll(
                'span', attrs={'class': 'label label-xs label-primary'})
            ...
            response = requests.request(
                "POST", post_url, data=payload, headers=headers)

            response_autocomplete = requests.request(
                "POST", post_autocomplete_url, data=payload_autocomplete, headers=headers)
